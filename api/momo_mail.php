<?php
    ...
    {
        $connection = imap_open('{imap.gmail.com:993/imap/ssl/novalidate-cert}INBOX', $ĐGB->site('email'), $ĐGB->site('pass_email')) or die('Cannot connect to Gmail: ' . imap_last_error());
        ...
        if (!empty($emailData))
        {
            foreach ($emailData as $emailIdent)
            {
                ...
                if ($id)
                {
                    ...
                    if($row['id'])
                    {
                        if($ĐGB->num_rows(" SELECT * FROM `momo` WHERE `tranId` = '$tranId' ") == 0)
                        {
                            $create = $ĐGB->insert("momo", array(
                                'tranId'        => $tranId,
    ...
    die('WEBSITE ĐƯỢC VẬN HÀNH BỞI BENTAN CODER FB : FB.COM/100068553914709 | ZALO : 0906287237');